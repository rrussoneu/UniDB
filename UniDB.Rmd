---
title: "University DB Implementation"
...
SELECT *, department_code || course_code || semester_offered AS composite_key
FROM Courses;
```
...
```{r}
get.dep <- "select distinct DepartmentCode,DepartmentName from `stu.data.raw`"
dep.info <- sqldf(get.dep)
nrows <- nrow(dep.info)

sql <- "insert into Departments (department_id, department_name, department_code) values"
#fac.info
#nrows
for (i in 1:nrows) {
  id <- i
...
#dep.id <- dbGetQuery(dbcon, "select department_id from departments where department_code = 'HIST'")
...
```
```{sql connection=dbcon}
SELECT * FROM Classrooms ORDER BY classroom_id DESC
```
