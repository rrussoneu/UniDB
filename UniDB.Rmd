...
```{sql connection=dbcon}
WITH percents as (
SELECT student_id, AvgGrade, NTILE(10) OVER (ORDER BY AvgGrade) as nt FROM UniAverageGrades
)
SELECT p.student_id, s.first_name, s.last_name, s.email from percents p, students s where p.nt > 7 and p.student_id = s.student_id ORDER BY p.AvgGrade DESC
```
Get only the students who qualify for summa cum laude (first 5 percent):
```{sql connection=dbcon}
WITH percents as (
SELECT student_id, AvgGrade, NTILE(20) OVER (ORDER BY AvgGrade) as nt FROM UniAverageGrades
)
SELECT p.student_id, s.first_name, s.last_name, s.email from percents p, students s where p.nt = 20 and p.student_id = s.student_id ORDER BY p.AvgGrade DESC
```
...
SELECT p.student_id, s.first_name, s.last_name, s.email from percents p, students s where p.nt < 20 and p.nt > 17 and p.student_id = s.student_id ORDER BY p.AvgGrade DESC
```
...
SELECT p.student_id, s.first_name, s.last_name, s.email from percents p, students s where p.nt < 18 and p.nt > 14 and p.student_id = s.student_id  ORDER BY p.AvgGrade DESC
```
